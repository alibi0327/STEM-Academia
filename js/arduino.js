...
async function init(){
 const a=await requireRole("teacher"); if(!a)return;
 session=a.session; $("logout").onclick=logout;
 let r=await supabase.from("courses").select("id").eq("slug","arduino").single();
 if(r.error)throw r.error; course=r.data;
 let asg=await supabase.from("teacher_courses").select("id").eq("teacher_id",session.user.id).eq("course_id",course.id).maybeSingle();
 if(!asg.data){alert("Курс Arduino не назначен.");return location.replace("teacher.html")}
 r=await supabase.from("lessons").select("id,title,lesson_order").eq("course_id",course.id).eq("active",true).order("lesson_order");
 rows=r.data||[];
 r=await supabase.from("lesson_progress").select("lesson_id,status").eq("teacher_id",session.user.id);
 (r.data||[]).forEach(x=>x.status==="completed"&&done.add(x.lesson_id));
 let n=rows.findIndex(x=>!done.has(x.id)); current=n>=0?n:lessons.length-1; render();
}
function render(){
 ...
 document.querySelectorAll(".lesson-link").forEach(b=>b.onclick=()=>{current=+b.dataset.i;render()});
 const pct=rows.length?Math.round(rows.filter(x=>done.has(x.id)).length/rows.length*100):0;
 $("courseProgress").style.width=pct+"%";$("courseProgressText").textContent=pct+"% завершено";
 $("finalTestBox").innerHTML=current===lessons.length-1&&isDone(current)?'<div class="card panel"><h2>Итоговый тест Arduino</h2><p class="muted">Все занятия завершены.</p><a class="btn" href="arduino-test.html">Перейти к тесту</a></div>':"";
}
