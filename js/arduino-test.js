...
import {questions} from "./arduino-test-data.js";
...
async function init(){
 ...
 let r=await supabase.from("courses").select("id").eq("slug","arduino").single();if(r.error)throw r.error;course=r.data;
 ...
 if((count||0)<(total||0)){alert("Сначала завершите все занятия.");return location.replace("arduino.html")}
 render();
}
function render(){
...
async function finish(){
 ...
 let r=await supabase.from("test_results").insert({teacher_id:session.user.id,lesson_id:last?.id||null,score,answers:{selected:sel,passed,course:"arduino"}});
 if(r.error)return alert(r.error.message);
 let cert=null;
 if(passed){r=await supabase.rpc("issue_course_certificate",{p_course_slug:"arduino"});if(!r.error)cert=r.data}
 $("questionCard").classList.add("hidden");$("resultBox").classList.remove("hidden");
 $("resultBox").innerHTML=`<h2>${passed?"Тест пройден":"Тест не пройден"}</h2><div class="score">${score}%</div><p>${correct} из ${questions.length} правильных ответов</p><div class="notice ${passed?"success":"warning"}">${passed?"Курс Arduino успешно завершен.":"Для сертификата необходимо минимум 80%."}</div><div class="form-actions" style="justify-content:center">${passed&&cert?`<a class="btn success" href="certificate.html?id=${encodeURIComponent(cert)}">Открыть сертификат</a>`:'<a class="btn" href="arduino-test.html">Пройти повторно</a>'}<a class="btn secondary" href="teacher.html">В кабинет</a></div>`;
}
