...
import {questions} from "./esp32-test-data.js";
...
async function init(){
 ...
 let r=await supabase.from("courses").select("id").eq("slug","esp32-iot").single();if(r.error)throw r.error;course=r.data;
 let asg=await supabase.from("teacher_courses").select("id").eq("teacher_id",session.user.id).eq("course_id",course.id).maybeSingle();
 if(!asg.data){alert("Курс ESP32 и IoT не назначен.");return location.replace("teacher.html")}
 ...
 if((count||0)<(total||0)){alert("Сначала завершите все занятия.");return location.replace("esp32.html")}
 render();
}
function render(){
...
async function finish(){
 ...
 let r=await supabase.from("test_results").insert({teacher_id:session.user.id,lesson_id:last?.id||null,score,answers:{selected:sel,passed,course:"esp32-iot"}});
 if(r.error)return alert(r.error.message);
 let cert=null;
 if(passed){r=await supabase.rpc("issue_course_certificate",{p_course_slug:"esp32-iot"});if(!r.error)cert=r.data}
 $("questionCard").classList.add("hidden");$("resultBox").classList.remove("hidden");
 $("resultBox").innerHTML=`<h2>${passed?"Тест пройден":"Тест не пройден"}</h2><div class="score">${score}%</div><p>${correct} из ${questions.length} правильных ответов</p><div class="notice ${passed?"success":"warning"}">${passed?"Курс ESP32 и IoT успешно завершен.":"Для сертификата необходимо минимум 80%."}</div><div class="form-actions" style="justify-content:center">${passed&&cert?`<a class="btn success" href="certificate.html?id=${encodeURIComponent(cert)}">Открыть сертификат</a>`:'<a class="btn" href="esp32-test.html">Пройти повторно</a>'}<a class="btn secondary" href="teacher.html">В кабинет</a></div>`;
}
